...
const { users, meta, loading, error, success } = storeToRefs(userStore)
    ...
    <div class="flex flex-col flex-1 rounded-[20px] p-5 gap-6 bg-white">
        <div class="header flex items-center justify-between">
            <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center gap-1">
                    <img src="@/assets/images/icons/profile-2user-grey.svg" class="flex size-6 shrink-0" alt="icon">
        ...
        <section id="List-Transactions" class="flex flex-col flex-1 gap-6 w-full">
            <div class="list flex flex-col gap-5">
                <CardList v-for="user in users" :key="user.id" :item="user" @delete="handleDelete"
                    v-if="!loading && users" />
            </div>
            <Pagination :meta="meta" :server-options="serverOptions" />
        </section>
        <div id="Empty-State" class="flex flex-col flex-1 items-center justify-center gap-4" v-if="users?.length === 0">
